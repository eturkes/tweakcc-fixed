 ...
 *   2.1.131..2.1.157 helper trimmed down to tab-only
 *   2.1.158+        third destructured param `tabAwareSeparator:VAR=!1` added;
 *                   separator chosen per-call (tab vs ":")
 *
 * Stable across all of these is the preamble:
 *   `{content:VAR,startLine:VAR2[,...extra]}){if(!VAR)return"";`
 * We anchor on that — tolerating any additional destructured params after
 * startLine — then splice in `return VAR` and erase the rest of the function
 * body up to its closing brace.
 */
